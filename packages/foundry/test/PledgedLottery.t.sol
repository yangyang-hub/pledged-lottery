...
contract PledgedLotteryTest is Test {
    ...
    function testInitialState() public view {
        ...
        (uint256 totalRevenue, uint256 totalPrizePaid, uint256 balance) = pledgedLottery.getContractStats();
    ...
    function testBuyTicket() public {
        ...
        (bool isScratched, uint256 prizeType, uint256 prizeAmount, bool isPrizeClaimed,) = pledgedLottery.getTicketInfo(1);
    ...
    function testQueryFunctions() public {
        ...
        (uint256 totalRevenue, , uint256 contractBalance) = pledgedLottery.getContractStats();
    ...
    function testMultipleBuyAndScratch() public {
        // Test buying multiple tickets and scratching them
        vm.startPrank(user1);
        ...
        uint256[] memory userTickets = pledgedLottery.getUserTickets(user1);
        assertEq(userTickets.length, 5);

        // Scratch all tickets
        for (uint256 i = 0; i < 5; i++) {
            pledgedLottery.scratchTicket(userTickets[i]);
        }

        // Check all tickets are scratched
        for (uint256 i = 0; i < 5; i++) {
            (bool isScratched,,,,) = pledgedLottery.getTicketInfo(userTickets[i]);
            assertTrue(isScratched);
        }

        vm.stopPrank();
    }

    function testBatchTicketPurchase() public {
        // Test buying multiple tickets in one transaction with higher value
        vm.startPrank(user1);

        uint256 ticketCount = 5;
        uint256 totalValue = TICKET_PRICE * ticketCount;

        pledgedLottery.buyTicket{value: totalValue}();

        uint256[] memory userTickets = pledgedLottery.getUserTickets(user1);
        assertEq(userTickets.length, ticketCount);

        // Check that total revenue increased correctly
        (uint256 totalRevenue,,) = pledgedLottery.getContractStats();
        assertEq(totalRevenue, totalValue);
